...
class AugmentationPipeline:
    ...
    def __init__(self, args):
        self.args = args
        self.weight_dict ={
                      'translation': args.aug_weight_translation,
                      'rotation': args.aug_weight_rotation,
                      'scaling': args.aug_weight_scaling,
                      'shearing': args.aug_weight_shearing,
                      'rot90': args.aug_weight_rot90,
                      'fliph': args.aug_weight_fliph,
                      'flipv': args.aug_weight_flipv
        }
        self.upsample_augment = args.aug_upsample
        self.upsample_factor = args.aug_upsample_factor
        self.upsample_order = args.aug_upsample_order
        self.transform_order = args.aug_interpolation_order
    ...
    def augment_image(self, im, max_output_size=None):
        ...
        if self.random_apply('translation'):
            h, w = im.shape[-2:]
            t_x = self.rng.uniform(-self.args.aug_max_translation_x, self.args.aug_max_translation_x)
            t_x = int(t_x * h)
            t_y = self.rng.uniform(-self.args.aug_max_translation_y, self.args.aug_max_translation_y)
            t_y = int(t_y * w)
        ...
        if self.random_apply('rotation'):
            interp = True
            rot = self.rng.uniform(-self.args.aug_max_rotation, self.args.aug_max_rotation)
        else:
            rot = 0.
        ...
        if self.random_apply('shearing'):
            interp = True
            shear_x = self.rng.uniform(-self.args.aug_max_shearing_x, self.args.aug_max_shearing_x)
            shear_y = self.rng.uniform(-self.args.aug_max_shearing_y, self.args.aug_max_shearing_y)
        else:
            shear_x, shear_y = 0., 0.
        ...
        if self.random_apply('scaling'):
            interp = True
            scale = self.rng.uniform(1-self.args.aug_max_scaling, 1 + self.args.aug_max_scaling)
        else:
            scale = 1.
...
class DataAugmentor:
    ...
    def __init__(self, args, current_epoch_fn):
        ...
        self.args = args
        self.aug_on = args.aug_on
        if self.aug_on:
            self.augmentation_pipeline = AugmentationPipeline(args)
        self.max_train_resolution = args.max_train_resolution
